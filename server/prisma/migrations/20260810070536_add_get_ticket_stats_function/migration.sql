-- Computes the dashboard stats (server/routes/tickets.ts GET /stats) entirely in the database
-- rather than in JS, so the aggregation runs as a single round-trip.
--
-- NEW/PROCESSING are transient AI-triage states hidden from the main Tickets list — this must be
-- kept in sync with HIDDEN_TICKET_STATUSES in server/routes/tickets.ts.
